...
def add_developer_points(
    ...
    query = """
        ...
        DO UPDATE
        SET
            amount = DeveloperPoints.amount + ((CAST((%(current_time)s - DeveloperPoints.last_update_time) AS bigint) * %(amount)s) / CAST(60 AS bigint)),
...
def nft_collection_exists(collection: str):
    query = f"""SELECT * FROM pg_tables
                 WHERE schemaname='nft_collections' AND tablename='{collection.lower()}_content'"""
    with __tongochi_db_connection.cursor() as cursor:
